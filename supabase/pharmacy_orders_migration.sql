...
create policy "Allow all for authenticated" on public.pharmacy_orders for all using (true);
...
add table public.pharmacy_orders;
-- Enable Realtime for completed cashier sales used by the admin dashboard.
do $$ begin if not exists (
  select 1
  from pg_publication_tables
  where pubname = 'supabase_realtime'
    and schemaname = 'public'
    and tablename = 'completed_sales'
) then alter publication supabase_realtime
add table public.completed_sales;
end if;
end $$;
