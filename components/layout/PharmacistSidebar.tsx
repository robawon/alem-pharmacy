...
export function PharmacistSidebar() {
  const { currentUser, prescriptions, logout } = useStore();
  const router = useRouter();
  const pathname = usePathname();
  const [mobileOpen, setMobileOpen] = useState(false);

  // Calculate pending prescriptions count for dynamic badge
  ...
  const pharmacistName = currentUser?.name || "Dr. Bethel Alemu";

  const PHARMACIST_NAV_ITEMS = [
    {
      href: "/pharmacist",
      label: "Rx Verification Queue",
      icon: Stethoscope,
    ...
    {
      href: "/pharmacist/interactions",
      label: "Drug Interactions DB",
      icon: AlertCircle,
    },
    {
      href: "/pharmacist/quarantine",
      label: "Batch Quarantines",
      icon: Lock,
    },
  ];
    ...
    <div className="flex h-full flex-col bg-slate-900 text-white border-r border-slate-800 p-4">
      ...
      <nav className="flex flex-1 flex-col gap-1.5 overflow-y-auto">
        ...
        {PHARMACIST_NAV_ITEMS.map((item) => {
          ...
          const isActive =
            pathname === item.href ||
            (item.href !== "/pharmacist" && pathname.startsWith(item.href));
          ...
          return (
            ...
            >
              ...
              {item.badgeCount !== undefined && item.badgeCount > 0 && (
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-white text-[11px] font-bold animate-pulse shadow-sm shadow-red-500/50">
                  {item.badgeCount}
                </span>
              )}
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3.5 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col min-w-0 flex-1">
              <p className="text-sm font-semibold text-white truncate">{pharmacistName}</p>
